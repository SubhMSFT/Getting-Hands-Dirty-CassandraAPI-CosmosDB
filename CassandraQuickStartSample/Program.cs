    ...
    public class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            */
            ...
            Console.WriteLine(String.Format("Successfully created keyspace weather with default 400 RU)."));
            ...
            Console.WriteLine("How do we find out the Request Unit (RU) charges for the operations using Azure Cosmos DB API for Cassandra?");
            Console.WriteLine("Proceed to find out...");

            Console.ReadLine();

            // When you perform operations against the Azure Cosmos DB Cassandra API, the RU charge is returned in the incoming payload as a field named RequestCharge.
            // When you use the .NET SDK, you can retrieve the incoming payload under the Info property of a RowSet object.
            RowSet rowSet1 = session.Execute("Select * from uprofile.user;");
            double requestCharge1 = BitConverter.ToDouble(rowSet1.Info.IncomingPayload["RequestCharge"].Reverse().ToArray(), 0);
            Console.WriteLine("Select * from table uprofile.user operation consumed {0} RUs.\n", requestCharge1);
            Console.ReadLine();
            ...
            RowSet rowSet2 = session.Execute("Select * from uprofile.user where user_id = 3;"); 
            double requestCharge2 = BitConverter.ToDouble(rowSet2.Info.IncomingPayload["RequestCharge"].Reverse().ToArray(), 0);
            Console.WriteLine("Select * from table uprofile.user where user_id = 3 operation consumed {0} RUs.\n", requestCharge2);
            ...
            RowSet rowSet3 = session.Execute("Select * from data where station_id = 1;");
            double requestCharge3 = BitConverter.ToDouble(rowSet3.Info.IncomingPayload["RequestCharge"].Reverse().ToArray(), 0);
            Console.WriteLine("Select * from table weather.data where station_id = 1 operation consumed {0} RUs.\n", requestCharge3);
